...
// Stage 3N: v3m DOT20 / OUT5 with targeted j-term timing relief.
//
// v3m proved that DOT20 / OUT5 is a strong cycles shape:
//   3733 HLS cycles, row loop 37 cycles/row.
// It also routed cleanly at 4.6 ns but missed setup by about 0.125 ns.
// The routed worst paths were compute-side B operand generation paths:
//   j_base -> k + j - seed -> DSP48E1 input register.
//
// This variant keeps DOT20 / OUT5 and the v3i/v3m row ping-pong datapath, but
// localizes the generated B/C terms into short-width per-output-lane registers
// before feeding the unrolled multiply fabric.
