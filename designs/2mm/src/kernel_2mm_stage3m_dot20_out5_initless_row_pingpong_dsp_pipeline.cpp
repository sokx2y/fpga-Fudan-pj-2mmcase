...
// Stage 3M: v3i datapath with a different compute shape: DOT20 / OUT5.
//
// v3i's DOT25 / OUT4 shape already routes at 4.6 ns with 4238 cycles and
// 800 DSPs.  This variant keeps the same successful structure:
//   initless A/B/C, row ping-pong DATAFLOW, no full tmp, no full D,
//   lane_sum reduction, complete row buffers, and latency-2 DSP multiply.
//
// The only architectural change is the parallel shape:
//   DOT25 / OUT4 => 25 k-lanes * 4 output lanes = 100 products per stage
//   DOT20 / OUT5 => 20 k-lanes * 5 output lanes = 100 products per stage
//
// This keeps the nominal multiplier pressure similar while reducing output
// tiles from 25 to 20.  The goal is to see whether row latency/cycles improve
// without exceeding the XC7K325T physical routing boundary.
