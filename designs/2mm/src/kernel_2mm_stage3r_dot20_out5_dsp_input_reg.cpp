...
// Stage 3R: v3q plus a one-step DSP input pipeline experiment.
//
// Keep the v3q champion architecture:
//   DOT20 / OUT5
//   initless A/B/C
//   row ping-pong DATAFLOW
//   no full tmp or D storage
//   ap_int<24> local dot accumulators
//   per-output-lane A/tmp operand copies
//
// Local experiment, not the V4 architectural mainline:
//   Ask HLS for a slightly deeper DSP multiply pipeline.  The v3q routed DRC
//   reports DPIP-1 warnings on DSP48 A inputs and the routed WNS is only
//   +0.006 ns at 4.6 ns.  This variant spends at most a small amount of row
//   latency to see whether DSP input-side timing improves enough to close
//   4.55 ns or create useful 4.6 ns margin.
...
static void compute_tmp_row(int i, short seed, DATA_TYPE tmp_row[NJ])
{
  ...
  for (int j0 = 0; j0 < NJ; j0 += OUT_UNROLL_FACTOR) {
    ...
    dot_acc_t acc[OUT_UNROLL_FACTOR];
    DATA_TYPE a_lane_local[OUT_UNROLL_FACTOR];
    DATA_TYPE b_lane[OUT_UNROLL_FACTOR];
    DATA_TYPE j_minus_seed[OUT_UNROLL_FACTOR];
    #pragma HLS ARRAY_PARTITION variable=acc complete dim=1
    #pragma HLS ARRAY_PARTITION variable=a_lane_local complete dim=1
    ...
    for (int k = 0; k < NK; ++k) {
      ...
      for (int jo = 0; jo < OUT_UNROLL_FACTOR; ++jo) {
        #pragma HLS UNROLL
        a_lane_local[jo] = a_lane;
        b_lane[jo] = add_short(k_local, j_minus_seed[jo]);
      }

      for (int jo = 0; jo < OUT_UNROLL_FACTOR; ++jo) {
        #pragma HLS UNROLL
        acc[jo] = add_product_narrow(acc[jo], a_lane_local[jo], b_lane[jo]);
      }
    }

    for (int jo = 0; jo < OUT_UNROLL_FACTOR; ++jo) {
      #pragma HLS UNROLL
      tmp_row[j_base + jo] = (DATA_TYPE)acc[jo];
    }
  }
}
    ...
    int lane_sum[OUT_UNROLL_FACTOR])
{
  ...
  for (int j0 = 0; j0 < NL; j0 += OUT_UNROLL_FACTOR) {
    #pragma HLS PIPELINE II=1
    dot_acc_t acc[OUT_UNROLL_FACTOR];
    DATA_TYPE tmp_lane_local[OUT_UNROLL_FACTOR];
    DATA_TYPE c_lane[OUT_UNROLL_FACTOR];
    DATA_TYPE seed_minus_j[OUT_UNROLL_FACTOR];
    #pragma HLS ARRAY_PARTITION variable=acc complete dim=1
    #pragma HLS ARRAY_PARTITION variable=tmp_lane_local complete dim=1
    ...
    for (int k = 0; k < NJ; ++k) {
      ...
      for (int jo = 0; jo < OUT_UNROLL_FACTOR; ++jo) {
        #pragma HLS UNROLL
        tmp_lane_local[jo] = tmp_lane;
        c_lane[jo] = add_short(k_local, seed_minus_j[jo]);
      }

      for (int jo = 0; jo < OUT_UNROLL_FACTOR; ++jo) {
        #pragma HLS UNROLL
        acc[jo] = add_product_narrow(acc[jo], tmp_lane_local[jo], c_lane[jo]);
      }
    }

    for (int jo = 0; jo < OUT_UNROLL_FACTOR; ++jo) {
      #pragma HLS UNROLL
      const DATA_TYPE d_val = (DATA_TYPE)acc[jo];
      lane_sum[jo] += d_val;
    }
  }
}
