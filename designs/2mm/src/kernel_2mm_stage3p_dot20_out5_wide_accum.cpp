...
// Stage 3P: v3n DOT20 / OUT5 with local wide accumulation.
//
// v3o showed that latency-binding the short accumulator add destroys HLS
// throughput.  This version keeps the fast DOT20 / OUT5 row-pingpong structure
// and changes only the dot-product accumulator representation:
//
//   product is still truncated to DATA_TYPE (short)
//   local dot-product sum is accumulated in int
//   final tmp/D value is cast back to DATA_TYPE (short)
//
// For addition, repeated 16-bit truncation and one final 16-bit truncation are
// equivalent modulo 2^16 as long as the wide accumulator does not overflow.
// With 100 short products, int is enough for this benchmark's value range.
...
static int add_product_wide(int acc, DATA_TYPE lhs, DATA_TYPE rhs)
{
  #pragma HLS INLINE
  const DATA_TYPE product = mul_dsp_short(lhs, rhs);
  return acc + (int)product;
}
...
static void compute_tmp_row(int i, short seed, DATA_TYPE tmp_row[NJ])
{
  ...
  for (int j0 = 0; j0 < NJ; j0 += OUT_UNROLL_FACTOR) {
    ...
    for (int k = 0; k < NK; ++k) {
      ...
      for (int jo = 0; jo < OUT_UNROLL_FACTOR; ++jo) {
        #pragma HLS UNROLL
        acc[jo] = add_product_wide(acc[jo], a_lane, b_lane[jo]);
      }
    }
    ...
    int lane_sum[OUT_UNROLL_FACTOR])
{
  ...
  for (int j0 = 0; j0 < NL; j0 += OUT_UNROLL_FACTOR) {
    ...
    for (int k = 0; k < NJ; ++k) {
      ...
      for (int jo = 0; jo < OUT_UNROLL_FACTOR; ++jo) {
        #pragma HLS UNROLL
        acc[jo] = add_product_wide(acc[jo], tmp_lane, c_lane[jo]);
      }
    }
