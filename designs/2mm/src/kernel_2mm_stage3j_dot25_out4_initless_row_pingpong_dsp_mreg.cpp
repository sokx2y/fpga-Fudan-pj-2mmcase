...
// Stage 3J: v3i plus a deeper DSP multiply pipeline experiment.
//
// Keep the v3i/v3g architecture:
//   DOT25 / OUT4, initless A/B/C, row ping-pong DATAFLOW, no full tmp,
//   no full D, lane_sum reduction, and complete row-buffer partitioning.
//
// Local experiment:
//   v3i routed at 4.8 ns but Vivado reported many DPOP-2 warnings that DSP
//   multiplier MREG output pipelining was not used.  This version keeps the
//   same algorithm and asks HLS for a deeper DSP multiply latency.  The product
//   is kept as int until the final accumulation cast, matching the original
//   short compound-assignment modulo behavior.
...
static int mul_dsp_pipelined(DATA_TYPE lhs, DATA_TYPE rhs)
{
  #pragma HLS INLINE
...
{
  #pragma HLS INLINE
  const int product = mul_dsp_pipelined(lhs, rhs);
