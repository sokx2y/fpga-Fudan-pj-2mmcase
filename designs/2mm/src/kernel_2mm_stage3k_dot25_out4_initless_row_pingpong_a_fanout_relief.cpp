...
// Stage 3K: v3i plus A-row fanout relief.
//
// Keep the v3i champion architecture:
//   DOT25 / OUT4, initless A/B/C, row ping-pong DATAFLOW, no full tmp,
//   no full D, lane_sum reduction, complete row-buffer partitioning, and
//   latency-2 DSP multiply binding.
//
// Local experiment:
//   v3i's routed timing paths are largely route dominated.  Some near-critical
//   paths also involve high-fanout row index signals feeding the compute-side
//   DSP array.  This version materializes the current A row as 100 completely
//   partitioned local values before the j-tile loop, so the DOT25/OUT4 compute
//   array reads local A registers/wires instead of repeatedly forming
//   i + k + seed across the pipelined multiply fabric.
...
static void build_a_row(int i, short seed, DATA_TYPE a_row[NK])
{
  #pragma HLS INLINE
...
static void compute_tmp_row(int i, short seed, DATA_TYPE tmp_row[NJ])
{
  ...
  for (int j0 = 0; j0 < NJ; j0 += OUT_UNROLL_FACTOR) {
    ...
    for (int k = 0; k < NK; ++k) {
      ...
      const int k_local = k;
      const DATA_TYPE a_lane = a_row[k_local];
