...
// Stage 3G: timing-relief version of the v3f champion.
// It preserves DOT25/OUT4, initless A/B/C, row ping-pong DATAFLOW, no full tmp,
// no full D, and lane_sum reduction.  The changes are intentionally local:
// remove constant-true bounds checks and expose per-lane input values before
// the multiply-adds so HLS/Vivado have more freedom to place/register DSP inputs.
